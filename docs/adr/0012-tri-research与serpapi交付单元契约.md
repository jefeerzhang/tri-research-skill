...
决定：开跑一次 Research Session 的**最小安装集合**是 `tri-research` + `serpapi`（`research-subagent` 推荐，`citations` 可选）。只装 `tri-research` **不足以** `state_machine start`。共享骨架已由 ADR-0015 收进 `tri_research_runtime`；Evidence Ledger / Registry 的 scripts 路径仍在。
...
- **本波落地 ADR-0015 共享 wheel**：当时被拒——解耦 import 路径是下一决策。**已由 ADR-0015 落地骨架包**；Ledger / Registry 路径仍在。
...
- **有意不动（当时）**：`required_backends.py` / Backend readiness（W2）；SerpApi 源码搬家（D2）；降档（D3）；共享 wheel（后由 ADR-0015 增量落地）；`architecture.json` 重绘。
