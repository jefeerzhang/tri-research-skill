...
- **（采用）`Backend.requirement` + `readiness()` + `SciVerseReadiness` 同一轮询列表**：改档只改字段；Exa 就绪与 `client()` 同一判断；SciVerse 留在列表外的 Registry。

## Consequences

- `BackendRequirementLevel`（`required` / `recommended` / `optional`）是可执行枚举，住在 `_search_cli`（与 `Backend` 同模块）。
- Exa：`requirement=required`，`readiness()` 只转 `client()`（K+S，无 start 探活）。SDK 与 key 同时缺时只报 SDK（与其余泳道同一顺序，ADR-0002 / 0008）。
