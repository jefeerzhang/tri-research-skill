...
报告正文可**可选**内嵌一张机制图/结构图，以 base64 data-URI 形式写进 md（`![图题](data:image/png;base64,...)`），使 md **单文件自包含**、无外部依赖，放在 `## 概述` 顶部或机制描述处，正文配一句文字指回。**验证器不要求、也不审计这张图**——它是推荐层的纯展示物。图源 `.drawio` 与导出的 `.png` 属中间产物，**定稿后删除**，最终只保留 md（图已内嵌，删除外部文件不会坏图）。图只能表达正文已出现的结论与关系，不新增内容。

## PDF 伴随产物（可选）

`done` 后可选用 `scripts/render_tex.py` 把报告渲成书样 PDF 作为交付伴随物；渲染时**自动跳过**报告内嵌的机制图（drawio 框架图），故 PDF 不含图。PDF 是派生产物，md 为唯一真源。
