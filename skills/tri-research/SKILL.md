...
**无子代理时的源使用**：当不派子代理时（见第二步决策表），Lead Agent 直接使用**所有可用源**（AnySearch + SciVerse + Exa + SerpApi + WebSearch）进行全面检索，不因无子代理而减少源覆盖。
...
**预检规则**：派子代理前运行 `python scripts/exa_search.py check`，返回 `{"available": true}` 则可用。
...
所有子代理返回后（或无子代理时 Lead Agent 完成检索后），向用户呈现结果摘要并等待确认：
...
用户确认「继续综合报告」后才进入下一步。用户也可指定补搜范围。
...
- 不泄露 API Key，不写外部数据
- 子代理可调用 AnySearch + SciVerse + Exa + 可选 Tavily；SerpApi 仅 Lead Agent 调用
